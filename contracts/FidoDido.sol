...
contract FidoDido is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Pausable, Ownable, ERC721Burnable, ReentrancyGuard {
   // GOOD AND WORKING
...
//    @notice _walletMintCount for private phase while _walletMintCountPublic for public phase; 
   ...
   constructor(address initialOwner, string memory name, string memory symbol,string memory baseUri,bytes32  _merkleRoot)
       ERC721(name, symbol)
...
// Function to set/change  non-revealed URI for token metadata
   function setnonRevealedUri(string memory baseURI) public onlyOwner {
    ...
    function testMerkel(address to, bytes32[] calldata merkleProof) public  view returns (bool) {
   ...
   function safeMint(address to, bytes32[] calldata merkleProof) public payable nonReentrant {
        require(_isPrivatePhase ? _walletMintCount[to] < 2 : _walletMintCountPublic[to] < 1, "Mint limit reached");
        require(_isPrivatePhase ? msg.value >= PRIVATE_PHASE_PRICE : msg.value >= PUBLIC_PHASE_PRICE, "Wrong Ether value");
        require(_nextTokenId+1 < MAX_SUPPLY, "Max supply reached");
        require(!(_isPrivatePhase && _nextTokenId+1 > 6777), "Total supply limit reached during private phase");
       ...
       override(ERC721, ERC721Enumerable)
   {
    ...
    string memory _tokenURI = revealed ? string(abi.encodePacked(baseURI, tokenId.toString(), ".json")) : _nonRevealedUri;
   ...
   require(payable(recipient).send(address(this).balance), "Transfer failed.");
   }
